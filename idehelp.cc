...
void LoadHelpDesktop(ipstream & is)
{
  ushort version;
  ClearHelpDesktop();
  is >> version;
...
const char * const TIDEInfWindow::name = "\x11";

TStreamable *TIDEInfWindow::build()
{
...
static void Copy2Clip(char *b,long l)
{
  if (clipWindow)
     clipWindow->editor->insertBuffer( b,0,(unsigned)l,False,True);
}

void *TIDEInfWindow::read(ipstream &is)
{
  TWindow::read(is);
  is >> viewer;
...
void TIDEInfWindow::changeBounds(const TRect &r)
{
...
void TIDEInfWindow::handleEvent(TEvent &event)
{
...
static const InfoHelpRec *HelpstringFromCtx(ushort ctx)
{
  int i=0;
...
static void CheckHelpWindow(int open_closed)
{
  int closed;
  InfWindow = (TWindow *)message(TProgram::application, evBroadcast, cmInfoAnswer, NULL);
  if (InfWindow)
    return;
  if (!open_closed)
    return;
