...
TStatusLine *IDE::initStatusLine( TRect r )
{
  ...
  line = new THintStatusLine( r,
     *new TStatusDef( hcFileViewer, hcFileViewer )
      ...
      +*new TStatusItem( _("~Alt+X~ Quit"), kbAlX, cmQuit )
    +*new TStatusDef( hcProjectListBox, hcProjectListBox )
      +*new TStatusItem( _("~Ins~ Add"), kbInsert, cmAddProjectItem )
      +*new TStatusItem( _("~Del~ Delete"), kbDelete, cmDelProjectItem )
      +*new TStatusItem( _("~^O~ Local"), kbCtO, cmLocalOptions )
      +*new TStatusItem( _("~^I~ Includes"), kbCtI, cmShowIncludes )
      +*new TStatusItem( NULL, kbCtTab, cmShowIncludes )
      +*new TStatusItem( _("~F5~ Zoom"), kbF5, cmZoom )
      +*new TStatusItem( _("~Alt+F9~ Compile"), kbAlF9, cmCompile )
      +*new TStatusItem( _("~Alt+X~ Quit"), kbAlX, cmQuit )
    ...
    +*new TStatusDef(  hcMsgListBox, hcMsgListBox )
      ...
      +*new TStatusItem( _("~Alt+F9~ Compile"), kbAlF9, cmCompile )
      +*new TStatusItem( NULL, kbF10, cmMenu )
      +*new TStatusItem( _("~Del~ Delete"), kbDelete, cmClearMessage)
      +*new TStatusItem( _("~Ctrl+Del~ Delete all"), kbCtDelete, cmClearMessages)
      +*new TStatusItem( _("~Alt+X~ Quit"), kbAlX, cmQuit )
    +*new TStatusDef( hcWindowList, hcWindowList )
      +*new TStatusItem( _("~Enter~ Select"), kbEnter, cmSelectWindow )
      +*new TStatusItem( _("~Del~ Close"), kbDelete, cmDeleteWindow )
      +*new TStatusItem( NULL, kbCtF5, cmResize )
    +*new TStatusDef( hcIDEFileEditor, hcIDEFileEditor )
      ...
      +*new TStatusItem( _("~F10~ Menu"), kbF10, cmMenu )
      +*new TStatusItem( _("~Alt+X~ Quit"), kbAlX, cmQuit )
      +*new TStatusItem( NULL, kbCtJ, cmGotoLine )
      +*new TStatusItem( NULL, kbAlL, cmcSetLocalOptions )
    +*new TStatusDef( hcInfView, hcInfView )
      +*new TStatusItem( _("~F1~ Help on InfoViewer"), kbF1, cmInfHelp )
      +*new TStatusItem( _("~Alt+F1~ last help"), kbAlF1, cmLastHelp )
      +*new TStatusItem( _("~Alt+F10~ local menu"), kbAlF10, cmInfControl )
      +*new TStatusItem( NULL, kbF10, cmMenu )
      +*new TStatusItem( _("~^F1~ Bookmarks"), kbCtF1, cmInfBookM )
      +*new TStatusItem( NULL, kbCtI, cmInfPasteIn )
#ifdef INTERNAL_DEBUGGER
    +*new TStatusDef(hcBreakListDialog, hcBreakListDialog)
      +*new TStatusItem( _("~Ins~ Add"), kbInsert, cmNewBreak)
      +*new TStatusItem( _("~Del~ Delete"), kbDelete, cmDeleteBreak)
      +*new TStatusItem( _("~Enter~ Jump to source"), kbEnter, cmSelectBreak)
    +*new TStatusDef(hcInspect, hcInspect)
       +*new TStatusItem( _("~F1~ Help"), kbF1, cmHelp )
       +*new TStatusItem( _("~F5~ Zoom"), kbF5, cmZoom )
       +*new TStatusItem( NULL, kbEsc, cmClose )
    +*new TStatusDef(hcFunctionListBox,hcFunctionListBox)
      +*new TStatusItem( _("~F1~ Help"), kbF1, cmHelp )
      +*new TStatusItem( _("~Enter~ Jump to source"), kbCtF5, cmResize )
    +*new TStatusDef(hcFunctionInputLine,hcFunctionInputLine)
      +*new TStatusItem(_("~Ctrl+F1~ Select from a list"), kbCtF1, cmSelectFunction)
    +*new TStatusDef( hcWatchListBox, hcWatchListBox )
      +*new TStatusItem( _("~F1~ Help"), kbF1, cmHelp )
      +*new TStatusItem( _("~Ins~ Add"), kbInsert, cmAddWatchEntry )
      +*new TStatusItem( _("~Del~ Delete"), kbDelete, cmRemoveWatch )
    ...
    +*new TStatusDef( hcOptFlagList, hcAdaFlagList )
      ...
      +*new TStatusItem( _("~Space~ select"), kbSpace, cmEnter )
    ...
    +*new TStatusDef( hcDisassemblerViewer, hcDisassemblerViewer)
      +*new TStatusItem( _("~F1~ Help"), kbF1, cmHelp)
      +*new TStatusItem( _("~^G~ Goto"), kbCtG, cmGotoDisass)
      +*new TStatusItem( _("~Alt+F3~ Close"), kbAlF3, cmClose)
