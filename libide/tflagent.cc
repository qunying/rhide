...
void TFlagEntry::ToggleActive()
{
  ...
  else
  {
	...
	return;
      case INT_TYPE:
	if (data.ivalue<0) strcpy(buffer,"0");
	else sprintf(buffer,"%d",data.ivalue);
	...
	else sscanf(buffer,"%d",&data.ivalue);
	break;
      case OPTIONAL_INT_TYPE:
	if (data.ivalue<0) strcpy(buffer,"");
	else sprintf(buffer,"%d",data.ivalue);
	...
	if (data.svalue.value) strcpy(buffer,data.svalue.value);
	else buffer[0] = 0;
        if (ValidInputBox(_("You must type a string here"),label,buffer,255,
            val) != cmOK)
	{
	  activated = False;
	  return;
	...
	break;
      case CHAR_TYPE:
	strcpy(label,_("~V~alue"));
	if (data.cvalue) strcpy(buffer,data.cvalue);
	else buffer[0] = 0;
	if (ValidInputBox(_("You must type a character here"),label,buffer,255,
	  NULL) != cmOK)
	{
	  activated = False;
	  return;
	...
	if (ValidInputBox(_("You can type a character here"),label,buffer,255,
	  NULL) == cmOK)
	{
	  string_free(data.cvalue);
	  string_dup(data.cvalue,buffer);
...
void *TFlagEntry::read(ipstream & is)
{
  char temp;
  ushort version;
  is >> version;
...
void TFlagEntry::write(opstream & os)
{
  char temp;
  ...
  activated(False),hasoption(False),type(FLAG_TYPE)
{
  ...
  if (tmp)
  {
    ...
    *tmp = '%';
    ...
    {
      case 'd':
	if (tmp[1] == '?') type = OPTIONAL_INT_TYPE;
	else type = INT_TYPE;
	data.ivalue = -1;
	break;
      case 'c':
	if (tmp[1] == '?') type = OPTIONAL_CHAR_TYPE;
	else type = CHAR_TYPE;
	data.cvalue = NULL;
	break;
      case 's':
...
TFlagEntry::TFlagEntry(const TFlagEntry * flag)
{
  ...
  switch (type)
  {
    ...
    case CHAR_TYPE:
    case OPTIONAL_CHAR_TYPE:
      if (flag->data.cvalue) string_dup(data.cvalue,flag->data.cvalue);
      else data.cvalue = NULL;
      break;
    case STRING_TYPE:
    case OPTIONAL_STRING_TYPE:
      if (flag->data.svalue.allowed) string_dup(data.svalue.allowed,flag->data.svalue.allowed);
      else data.svalue.allowed = NULL;
      if (flag->data.svalue.value) string_dup(data.svalue.value,flag->data.svalue.value);
      else data.svalue.value = NULL;
      break;
    default:
      break;
  }
...
char * TFlagEntry::GetText()
{
  char * tmp;
  char buffer[256];
  string_dup(tmp,"[");
  if (activated == True) string_cat(tmp,"x] ");
  else string_cat(tmp," ] ");
  ...
  switch (type)
  {
    ...
    case STRING_TYPE:
    case OPTIONAL_STRING_TYPE:
      if (data.svalue.value) string_cat(tmp,data.svalue.value);
      break;
    case CHAR_TYPE:
    case OPTIONAL_CHAR_TYPE:
      if (data.cvalue) string_cat(tmp,data.cvalue);
      break;
    default:
      break;
  }
...
char * TFlagEntry::GetControlString()
{
  char *tmp;
...
char * TFlagEntry::GetValue()
{
  char * tmp;
  char itmp[10];
  if (activated == False) return NULL;
  string_dup(tmp,value);
  switch (type)
  {
    case FLAG_TYPE:
      return tmp;
    case OPTIONAL_STRING_TYPE:
      if (!data.svalue.value) return tmp;
    case STRING_TYPE:
      string_cat(tmp,data.svalue.value);
      return tmp;
    case OPTIONAL_INT_TYPE:
      if (data.ivalue < 0) return tmp;
    case INT_TYPE:
      sprintf(itmp,"%d",data.ivalue);
      string_cat(tmp,itmp);
      return tmp;
    case OPTIONAL_CHAR_TYPE:
      if (!data.cvalue) return tmp;
    case CHAR_TYPE:
