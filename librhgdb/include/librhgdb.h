...
#define B_ENABLED   0x0001 /* is this brakpoint enabled? */
#define B_LINE      0x0002 /* is it a breakpoint at a line number? */
#define B_FUNCTION  0x0004 /* is it a breakpoint at a function? */
#define B_CONDITION 0x0008 /* has this breakpoint a condition? */
#define B_COUNT     0x0010 /* has this breakpoint an ignore count? */
#define B_HARD      0x0020 /* is it a hardware breakpoint? */

struct BreakPoint
{
  unsigned short type; /* B_xxx above */
  int number; /* this is the breakpoint number used by GDB. Do not modify it*/
  unsigned long address; /* the real address of this breakpoint */
  char *filename; /* valid, if B_LINE, possible valid if B_FUNCTION */
  char *condition; /* valid, if B_CONDITION */
  char *function; /* valid, if B_FUNCTION */
  int count; /* valid, if B_COUNT */
  int line_number; /* valid, if B_LINE, possible valid if B_FUNCTION */
};
...
/*
  Return the size of processor register 'num' in bytes.
*/
