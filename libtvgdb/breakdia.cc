...
TBreakListDialog::TBreakListDialog(const TRect &bounds,const char *Title) :
  ...
  TWindowInit(TBreakListDialog::initFrame)
{
  TScrollBar *sb;
  TRect r,rr;
  int size;
...
void TBreakListDialog::handleEvent(TEvent &event)
{
...
void TBreakEditDialog::handleEvent(TEvent &event)
{
  TDialog::handleEvent(event);
  switch (event.what)
  {
    case evBroadcast:
      switch (event.message.command)
      {
        case cmRadioItemSelected:
          if (event.message.infoPtr != Radio) break;
          update();
          clearEvent(event);
          break;
...
TBreakEditDialog::TBreakEditDialog(const char *Title) :
  TDialog(TRect(0,0,51,14),Title),
  TWindowInit(TBreakEditDialog::initFrame)
{
...
static void EditBreak(int number)
{
  ushort type,control;
  ...
  if (bp)
  {
    if (BREAK_LINE(bp)) type = 0;
    else type = 1;
    if (bp->filename) dialog->File->setData(bp->filename);
    dialog->Line->setData(&bp->line_number);
    if (BREAK_FUNCTION(bp)) dialog->Function->setData(bp->function);
  ...
  if (control == cmOK)
  {
    ...
    if (*_buffer) bp->condition = strdup(_buffer);
...
void BreakDialog()
{
    ...
    switch (control)
    {
      ...
      case cmEnableBreak:
        ...
        if (dialog->bl->range > 0)
        {
          ...
          bp = GetBreakPoint(dialog->bl->focused);
          if (BREAK_ENABLED(bp)) DisableBreakPoint(foc);
          else EnableBreakPoint(foc);
          dialog->bl->update();
  ...
  destroy(dialog);
  if (bp && bp->filename
      && message(TProgram::application,evBroadcast,cmOpenWindow,bp->filename))
  {
