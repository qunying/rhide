...
TFuncDialog::TFuncDialog(const char *aTitle,const char *regex) :
  TDialog(TRect(0,0,70,22),aTitle),
  TWindowInit(TFuncDialog::initFrame)
{
...
void TFuncDialog::handleEvent(TEvent &event)
{
                      ...
                      char *GDB_name,function_entry **func)
{
  ...
  if (result == cmOK)
  {
    ...
    if (full_name) dialog->list->getText(full_name,foc,1024);
    if (GDB_name) strcpy(GDB_name,_func->linkage_name);
    if (func) *func = _func;
  }
  destroy(dialog);
  return result;
