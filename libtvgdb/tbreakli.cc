...
void TBreakListBox::update()
{
  setRange(BreakPointCount());
  drawView();
}

void TBreakListBox::getText(char *dest,ccIndex item,short maxLen)
{
  char buffer[256],*temp;
  char tmp[512];
  BreakPoint *bp = GetBreakPoint(item);
  memset(buffer,' ',maxLen);
  if (BREAK_ENABLED(bp)) buffer[0] = 251;
  temp = buffer+1;
  if (bp->filename) memcpy(temp,bp->filename,strlen(bp->filename));
  else memset(temp,' ',filesize);
  temp += filesize;
  *temp++ = delemiter;
  ...
  temp += linesize;
  *temp++ = delemiter;
  if (BREAK_CONDITION(bp)) memcpy(temp,bp->condition,strlen(bp->condition));
  else memset(temp,' ',condsize);
  temp += condsize;
  *temp++ = delemiter;
