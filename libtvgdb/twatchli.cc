...
void TWatchListBox::GetVariable(ccIndex item,char *buffer,int maxlen)
{
  strncpy(buffer,((TWatchEntry *)list()->at(item))->variable,maxlen);
  buffer[maxlen] = 0;
}

void TWatchListBox::SetVariable(ccIndex item,char *buffer)
{
...
void TWatchListBox::AddVariable(char *var)
{
  TWatchEntry *e;
  e = new TWatchEntry;
...
void TWatchListBox::RemoveVariable(ccIndex item)
{
...
void TWatchListBox::selectItem(ccIndex item)
{
  char buffer[256];
  if (item+1 < range)
  {
    ...
    if (HistinputBox(_("Change the expression to watch"),_("~E~xpression"),
                     buffer,255,tvgdb_History_Watch_Expression)
        == cmOK)
    {
...
void TWatchListBox::handleEvent(TEvent & event)
{
  switch (event.what)
  {
    case evKeyDown:
  ...
  switch (event.what)
  {
    case evCommand:
      ...
      {
        case cmEditWatch:
          if (focused+1 < range) selectItem(focused);
          clearEvent(event);
          break;
        case cmRemoveWatch:
          if (focused+1 < range) RemoveVariable(focused);
          clearEvent(event);
          break;
        default:
          break;
...
TWatchListBox::TWatchListBox(const TRect & bounds,
                             int numCols,TScrollBar *vbar,TScrollBar *hbar) :
...
void TWatchListBox::getText(char *dest,ccIndex index,short maxlen)
{
  int len;
  if (index+1 == range)
  {
  ...
  len += 2;
  if (len >= maxlen) return;
  if (e->expr) strncat(dest,e->expr,maxlen-len);
}
