...
static char * IdentifyBreakPoint(struct BreakPoint *bp,char *msg)
{
...
static char *merge_message(char *msg1,char *msg2)
{
  strcpy(buffer,msg1);
  strcat(buffer,msg2);
  return buffer;
...
void UserWarning(int number, ...)
{
  ...
  switch (number)
  {
    ...
    case WARN_NOCODE: // probably optimized out
      ...
      break;
    case WARN_NOPROGRAM: /* This can't be happen in RHIDE */
  ...
  if (msg)
  {
    if (strlen(msg) > 50) BigmessageBox(msg,mfError|mfOKButton);
    else messageBox(msg,mfError | mfOKButton);
  }
  va_end(arg);
}
