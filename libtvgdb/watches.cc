...
{
public:
  TWatchList(const TRect& bounds, const char *aTitle ) :
...
void TWatchList::changeBounds(const TRect &r)
{
  TDialog::changeBounds(r);
  WatchWindowRect = r;
}

void TWatchList::shutDown()
{
  TDialog::shutDown();
  watches = NULL;
}
...
void OpenWatchWindow()
{
  ...
  watches = new TWatchListBox(r,1,
             watchwindow->standardScrollBar(sbVertical | sbHandleKeyboard),
             watchwindow->standardScrollBar(sbHorizontal | sbHandleKeyboard));
...
void Evaluate(char *watch)
{
  TWatchDialog *d;
  d = new TWatchDialog(TRect(10,2,70,14),_("Evaluate and modify"),watch ? watch : "");
...
void DeleteAllWatches()
{
  ...
  {
    i--;
    if (watches) watches->RemoveVariable(i);
    Watches.atFree(i);
  }
}
