...
void RemoveWindow(TWindow *window, Boolean remember_closed)
{
  int i,count;
  if (!windows) return;
  if (!window) return;
...
TWindow *OpenClosedWindow(int nr)
{
  TWindow *window;
  closed_window *cw = (closed_window *)closed_windows->at(nr);
...
int IsClosedWindow(const char *name)
{
  if (!closed_windows)
    return -1;
  if (!name)
...
static void SaveClosedWindow(int nr, opstream &os)
{
  closed_window *cw = (closed_window *)closed_windows->at(nr);
...
void SaveClosedWindows(opstream &os)
{
  ccIndex count=0;
  int i;
  os << closed_version;
...
static void LoadClosedWindow(ipstream &is)
{
  ushort version;
  is >> version;
  closed_window *cw = new closed_window;
...
void LoadClosedWindows(ipstream &is)
{
  ushort version;
  int i;
  is >> version;
