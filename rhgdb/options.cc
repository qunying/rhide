     ...
     TWindowInit(TEnvironmentDialog::initFrame)
{
  ...
  tmp = _("~S~creen mode");
  insert(new TLabel(
    TRect(r.a.x,r.a.y-1,r.a.x+cstrlen(tmp)+1,r.a.y),tmp,radio));
  insert(radio);
  r.a.y = r.b.y;
  r.b.y = r.a.y + 1;
  r.b.x = r.a.x + 10;
  ...
  check = new TEnterCheckBoxes(r,new TSItem(_("Show G~D~B commands"),                     // d
                                 ...
                                 NULL)));
  if (verbose_gdb_commands) check_data |= (1 << 0);
  if (use_dual_display) check_data |= (1 << 1);
...
void Preferences()
{
  ...
  while (do_it && TProgram::deskTop->execView(dialog) == cmOK)
  {
    ...
    if (check_data & (1 << 0)) verbose_gdb_commands = 1;
    else verbose_gdb_commands = 0;
    if (check_data & (1 << 1)) use_dual_display = 1;
    else use_dual_display = 0;
